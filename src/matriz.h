...
class Matriz {
    class Zp {
       ...
       public:
        ...
        int value() { return _n; }
        Zp operator+(Zp t) { return Zp(_n + t._n); }
        Zp operator*(Zp t) { return Zp(_n * t._n); }
        Zp operator=(Zp t) {
            _n = t._n;
            return (*this);
        }
        bool operator!=(Zp t) { return _n != t._n; }
        bool operator==(Zp t) { return _n == t._n; }
    };
    Zp m[3][3];
    friend istream& operator>>(istream& i, Matriz& mat);
    friend ostream& operator<<(ostream& o, Matriz mat);
    int val(int i, int j) { return m[i][j].value(); }
   ...
   public:
    ...
    void operator*=(Matriz o) { (*this) = (*this) * o; }
    Matriz operator=(Matriz o) {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                m[i][j] = o.m[i][j];
    ...
    bool operator!=(Matriz o) { return !((*this) == o); }
};
...
ostream& operator<<(ostream& o, Matriz mat) {
    for (int i = 0; i < 3; i++) {
        o << mat.val(i, 0) << " " << mat.val(i, 1) << " " << mat.val(i, 2)
          << endl;
    }
    return o;
}
