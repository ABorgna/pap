...
\Function{calcular$\_$camino$\_$aumento}{capacidad_red_residual, G, source, sink}
	...
	\State \textbf{Si no hubo camino de aumento (no llegue al nodo final)}
	\If{not visited[sink]}
		\Return 0
	\EndIf

	\State \textbf{En caso de que sí, reconstruyo el camino y a todas esas aristas las marco como usadas y no pueden volver a usarse.}
	...
	\State \textbf{Toda arista tiene capacidad 1 \\}
	\Return 1
\EndFunction
\end{algorithmic}
\hspace{1cm}
...
\Function{main}{N, M, tipo$\_$esquina T[], lista$\_$incidencia L[]}
	\State \textbf{Vamos a tener a todo nodo del grafo original duplicado como V$_{entrante}$ y V$_{saliente}$}
	\State \textbf{Y el digrafo representado como una lista de adyacencia.}
	\State Tamaño(G) = 2 * N + 2
	\State \textbf{Por otro lado la red residual como una matriz de Tamaño(G) * Tamaño(G)}
