...
long long gHash(string s, long long seed) {
    assert(seed > (1LL << 32) && seed < (1LL << 56));
    vector<long long> primos;
    int n = s.size();
    for (int i = 2; (signed)primos.size() <= n; i++) {
        bool esPrimo = true;
        for (int j = 0; j < (signed)primos.size(); j++) {
            if (i % primos[j] == 0) {
                esPrimo = false;
                break;
            }
...
string integrantes(int grupo) {
    string apellidos[17] = {
        ...
        "Nale,Pinelli,Pontalti,Raffo(10)", "Bulavka,Fixman,Gutierrez,Levy(11)",
        "Bekier,Bokser,Borgna,Del Gobbo(12)",
        "Gambaccini,Penas,Pesaresi,Rissola(13)",
        "Bohe,Cadaval,Lew,Noli Villar(14)", "Bianchi,Jedwabny,Mena,Pondal(15)",
...
int main() {
    ...
    while (grupo < 1 || grupo > 17) {
        cout << "Dale! Media pila, como van a ser el grupo " << grupo
             << " si los numeros de grupo van de 1 a 17?" << endl;
        cin >> grupo;
    }
    ...
    while (seed <= (1LL << 32) || seed >= (1LL << 56)) {
        ...
        if (seed <= (1LL << 32)) {
            cout << "lo siento, " << seed
                 << "es muy chico, tiene que ser mayor a " << (1LL << 32)
                 << endl;
        } else if (seed >= (1LL << 56)) {
            cout << "lo siento, " << seed
                 << "es muy grande, tiene que ser menor a " << (1LL << 56)
                 << endl;
        }
    }
    long long num = gHash(integrantes(grupo), seed);

    cout << "Su numero de grupo para entregar es " << num
         << " y no se olviden que lo generaron con la semilla " << seed
         << ", van a necesitar guardar este numero" << endl;
